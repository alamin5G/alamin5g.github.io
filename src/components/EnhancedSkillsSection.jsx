...
import { Brain, Code, Database, Lightbulb, Play, Pause, RotateCcw } from 'lucide-react';
...
const EnhancedSkillsSection = ({ skillLevels }) => {
  ...
  const playAnimation = () => {
    ...
    const skills = Object.entries(skillLevels[activeCategory]);
    ...
    }, skills.length * 200 + 1600);
  };

  const resetAnimation = () => {
    Object.values(skillRefs.current).forEach(element => {
      if (element) {
    ...
    setTimeout(() => {
      Object.entries(skillLevels[activeCategory]).forEach(([skill, level], index) => {
        const skillKey = `${activeCategory}-${index}`;
        const element = skillRefs.current[skillKey];
        if (element) {
        ...
        <div className="relative">
          <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-3 overflow-hidden shadow-inner">
            <div
              ...
              className={`h-full bg-gradient-to-r ${categories[activeCategory].gradient} rounded-full relative overflow-hidden transition-all duration-300 group-hover:shadow-lg`}
              style={{ width: `${level}%` }}
            >
      ...
      <div className="container mx-auto max-w-6xl px-4 relative z-10">
        ...
        <div className="flex justify-center gap-4 mb-8">
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-green-500 hover:bg-green-600 disabled:bg-gray-400 text-white rounded-lg transition-colors duration-300"
          >
